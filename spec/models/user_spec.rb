  ...
  module Test
    module Utils # :nodoc:
      ...
      def build_nested_query(value, prefix = nil)
        case value
        when Array
          if value.empty? # patch!
            "#{prefix}[]=[]"
          else
            value.map do |v|
            prefix = "#{prefix}[]" unless unescape(prefix).match?(/\[\]$/)
...
RSpec.describe "API",:devise,:normal do
  before do
    @user = create(:user, name: "Example User", email: "user@example.com", admin: false)
    @michael = create(:user)
    @archer = create(:user, name: "Sterling Archer", email: "duchess@example.gov", admin: false)
    @lana = create(:user, name: "Lana Kane", email: "hands@example.gov", admin: false)
  end
  ...
  describe 'for user with different name' do
      before do
